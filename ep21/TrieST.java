...
public class TrieST<Value> {
   ...
   /**
    ...
    private void traversal(Node x, String prefix, Queue<String> q)
    {
        if (x == null) return;
        if (x.val != null) q.enqueue(prefix);
        for (char c = 0; c < R; c++)
            traversal(x.next[c], prefix + c, q);
    }

    /* retorna a maior string menor igual que s*/
    public String floor (String s) {
        ...
        for (String key : queue) {
            ...
            else if (key.compareTo(s) <= 0 && key.compareTo(maior) > 0) {
                maior = key;
            }
            i++;
        }
    ...
    /* retorna a menor string maior igual que x */
    public String ceil (String s) {
        ...
        for (String key : stack) {
            ...
            else if (key.compareTo(s) >= 0 && key.compareTo(menor) < 0) {
                menor = key;
            }
            i++;
        }
    ...
    /* retorna a posição de x na ordem lexicográfica*/
    public int rank (String s) {
        cont = 0;
        return rank (root, "", s);
    }

    private int rank (Node x, String prefix, String s) {
        if (x == null) return -1;
        else if (x.val != null && s.compareTo(prefix) == 0) return cont;
        else if (x.val != null) cont++;

        for (char c = 0; c < R; c++) {
            int i = rank (x.next[c], prefix + c, s);
            if (i != -1) return i;
        }
        return -1;
    }

    /* retorna a i-ésima String na ordem lexicográfica */
    public String select (int i) {
        cont = 0;
        return select (root, "", i);
    }

    private String select (Node x, String prefix, int i) {
        if (x == null) return null;
        else if (x.val != null && cont == i) return prefix;
        else if (x.val != null) cont++;

        for (char c = 0; c < R; c++) {
            String s = select (x.next[c], prefix + c, i);
            if (s != null) return s;
        }
        return null;
    } 
    ...
    public static void main(String[] args) {
        ...
        int i;
        while ((i = StdIn.readInt ()) != -1)
            StdOut.println ("select " + i + " " + st.select(i));
        
        String s;
        for (s = StdIn.readString (); s.compareTo ("fim") != 0; s = StdIn.readString ())  {
            StdOut.println ("rank " + s + " = " + st.rank(s));
            StdOut.println ("ceil " + s + " = " + st.ceil(s));
